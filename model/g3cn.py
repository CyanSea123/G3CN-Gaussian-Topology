...
class Model(nn.Module):
    ...
    def forward(self, x, label=None, **kwargs):
        ...
        x = self.l1(x,label=label)

        x = self.l2(x,label=label)
        x = self.l3(x,label=label)
        x = self.l4(x,label=label)
        x = self.l5(x,label=label)
        x = self.l6(x,label=label)
        x = self.l7(x,label=label)
        x = self.l8(x,label=label)

        x = self.l9(x,label=label)
        x = self.l10(x,label=label)
